...
from third_party.tvl.tvl_enc.tvl import TVL, ModalityType
from collections import OrderedDict

class LLaMA_adapter(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(self, llama_ckpt_dir, llama_tokenizer, phase="finetune", legacy_bridge=False, args=None, load=True):
        super().__init__()
        self.active_modality_names = args.active_modality_names

        feature_dim = 768
        self.image_bind = TVL(active_modalities = [ModalityType.VISION, ModalityType.TACTILE], tactile_model=args.tactile_model)

        if args.checkpoint_path is not None:
            state_dict = torch.load(args.checkpoint_path, map_location='cpu', weights_only=False)['model']
        ...
        model_args: ModelArgs = ModelArgs(
            max_seq_len=2048, max_batch_size=1, w_bias=bias_lora, w_lora=bias_lora,  **params
        )
        ...
        if phase == 'finetune':
            ...
            for name, para in self.named_parameters():
                if name.startswith("llama."):
                    if 'gate' in name:
                        trainable[name] = para
                elif name.startswith("image_bind_"):
                    trainable[name] = para
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        for k in state_dict:
            param = state_dict[k]
            if "image_bind." in k or k in self.llama_keys:
                continue
    ...
    def forward(self, tokens, labels, observations):
        ...
        visual_feats = self.forward_visual(imagebind_inputs)
...
def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts',
         llama_type="7B", phase="finetune", args=None):
    if name in _MODELS:
    ...
    model = LLaMA_adapter(
        llama_ckpt_dir, llama_tokenzier_path, phase=phase, args=args, load=True)
