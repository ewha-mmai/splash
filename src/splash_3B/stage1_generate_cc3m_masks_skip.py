...
parser.add_argument(
    ...
    type=float,
    default=0.6,
    help="Pruning ratio (e.g., 0.3 = 30% pruning)"
)
parser.add_argument(
    "--num_samples",
    type=int,
    default=128,
    help="Number of calibration samples (e.g., 64, 128, 256)"
)
...
SAVE_DIR = os.path.join(
    root_dir,
    f"src/splash_3B/masks/{int(PRUNING_RATIO * 100)}"
)
...
def load_cc3m_pairs(text_path, image_folder, num_samples):
    ...
    if os.path.isfile(text_path):
        ...
        files = sorted(glob.glob(os.path.join(text_path, "*.tsv"))) or \
                sorted(glob.glob(os.path.join(text_path, "*.json")))
        if not files:
            print(f"     ! ( : {text_path})")
            return pairs
        path = files[0]
...
def generate_weight_wanda_mask(model, processor):
    ...
    save_path = os.path.join(
        SAVE_DIR,
        f"mask_wanda_{int(PRUNING_RATIO*100)}_samples_{NUM_SAMPLES}_v2.pt"
    )
