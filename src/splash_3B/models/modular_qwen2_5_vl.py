...
from models.tactile_projector import TactileProjector
...
class Qwen2_5_VLModel(Qwen2VLModel):
    ...
    def __init__(self, config):
        ...
        # --- tactile encoder (ViT-Tiny) ---
        self.tactile_encoder = TactileEncoder(
            pretrained_path=getattr(config, "tactile_encoder_path", None)
        )

        # --- tactile projector (adapter) ---
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if tactile_pixel_values is not None:
            tactile_pixel_values = tactile_pixel_values.to(
                device=inputs_embeds.device,
                dtype=inputs_embeds.dtype,
            )

            # 1. ViT-Tiny encode
            tactile_feats = self.tactile_encoder(tactile_pixel_values)
            # [B, N_patch, 192]

            # 2. Project to LLM space
            tactile_tokens = self.tactile_projector(tactile_feats)
            # [B, N_patch, 3584]

            # 3. Inject into <tactile> tokens
            inputs_embeds = self._inject_mm_tokens(
                inputs_embeds,
                input_ids,
                tactile_tokens,
                modality="tactile",
...
class Qwen2_5_VLForConditionalGeneration(Qwen2VLForConditionalGeneration):
    ...
    def forward(
        ...
        tactile_pixel_values: torch.Tensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
...
class Qwen2_5_VLProcessor(Qwen2VLProcessor):
    ...
    def model_input_names(self):
        ...
        tactile_processor_input_names = self.tactile_processor.model_input_names

        names_from_processor = list(
            dict.fromkeys(tokenizer_input_names + image_processor_input_names + video_processor_input_names + tactile_processor_input_names)
        )
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2_5_VLProcessorKwargs],
        ...
        #     merge_length = self.tactile_processor.merge_size**2
        #     index = 0
        #     for i in range(len(text)):
        #         while self.tactile_token in text[i]:
        #             num_tactile_tokens = tactile_grid_thw[index].prod() // merge_length
        #             text[i] = text[i].replace(self.tactile_token, "<|placeholder|>" * num_tactile_tokens, 1)
        #             index += 1
        #         text[i] = text[i].replace("<|placeholder|>", self.tactile_token)
